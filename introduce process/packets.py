...
class PacketType(str, Enum):
    ...
    INTRODUCE = "010"
    INTRODUCE_ACK = "011"
    FETCH_INTRODUCER = "100"
    FETCH_INTRODUCER_ACK = "101"
...
class Packet:
    ...
    def pack(self) -> bytes:
        ...
        return struct.pack(f"i{255}s{3}si{2048}s", len(self.sender), self.sender.encode('utf-8'), self.type.encode('utf-8'), len(jsondata), jsondata.encode())
    ...
    def unpack(recvPacket: bytes):
        ...
        try:
            unpacked_tuple: tuple[bytearray] = struct.unpack(
                f"i{255}s{3}si{2048}s", recvPacket)
